async function getWeather() {
  var restcontUrl = "https://restcountries.eu/rest/v2/all";
  var contRequest = await fetch(restcontUrl);
  var data = await contRequest.json();
  var len = data.length;
...
async function getWeatherData(data) {
  try {
    ...
    // var contRequest = await fetch(restcontUrl);
    // var data = await contRequest.json();
    var latlong = data.latlng;
    var lat = latlong[0];
    var long = latlong[1];
    var url = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=60f30cfc2b59a80d13cec75b84a3db7d`;
    var weatherRequest = await fetch(url);
    var data1 = await weatherRequest.json();
