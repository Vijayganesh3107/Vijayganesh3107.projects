...
//API for displaying the data
app.get("/longURL", async (req, res) => {
  try {
    var client = await mongoclient.connect(url, { useUnifiedTopology: true });
    var db = client.db("assignment");
    var data = await db.collection("urls").find().toArray();
    res.json(data);
    client.close();
  } catch (error) {
    console.log(error);
...
app.put("/longURL", async (req, res) => {
  ...
  if (result == 1) {
    var resultdata = await db
      .collection("urls")
      .updateOne({ longURL: req.body.longURL }, { $inc: { clicked: 1 } });
    client.close();
    res.json({
